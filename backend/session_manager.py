...
class SessionManager:
    ...
    async def _wait_for_user_speech_idle(
        ...
        session: Session,
        max_wait: float = 8.0,
        post_silence_grace: float = float(os.getenv("POST_SILENCE_GRACE_S", "0.12")),
    ) -> bool:
        """
    ...
    async def _llm_tts_loop(
        ...
        while True:
            ...
            if await self._wait_for_user_speech_idle(session):
                _tail_merge_s = 0.2 if expecting_mobile_number else 0.12
                while True:
                    try:
                        more: TranscriptResult = await asyncio.wait_for(
                            session.transcript_queue.get(), timeout=_tail_merge_s
                        )
                    except asyncio.TimeoutError:
                        break
                    except asyncio.CancelledError:
            ...
            # cancel_and_wait_tts clearing the events and this new turn.
